...
void MIXTRAL::create_mixtral_model(FFModel &ff,
  ...
  for (int i = 0; i < mixtral_config.num_hidden_layers; i++) {
    ...
    // printf("TOPK VALUES DIM %d", topk_values->num_dims);
    ...
    topk_values = ff.add(topk_values, topk_values_reduced, false, std::string("layers." + std::to_string(i) +
                                       ".divide_topk")
                               .c_str()); // (2, 1, 128)
